...
const SideBar = ({isMenuOpen ,setIsMenuOpen}) => {
  ...
  return (
    ...
    >
      ...
      <img src={assets.search_icon} className='w-4 not-dark:invert' alt="searchicons" />
      ...
      {/* User Account */}
      <div className ="flex items-center gap-3 p-3 mt-4 border-gray-300 
      dark:border-white/15 rounded-md cursor-pointer">
        <img src={assets.user_icon} 
        className='w-7 rounded-full' alt ="usericon"/>
        <p className='flex-1 text-sm dark:text-primary truncate'>
            {user ? user.name : "Login to your account"}
        </p>
        {user && <img src={assets.logout_icon}
        className='h-5 cursor-pointer hidden:md not-dark:invert group-hover:block'
        />}
